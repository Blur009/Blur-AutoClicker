...
pub fn is_vk_down(vk: i32) -> bool {
    match vk {
        value if value == VK_LBUTTON as i32 => mac_mouse_button_down(0),
        value if value == VK_RBUTTON as i32 => mac_mouse_button_down(1),
        value if value == VK_MBUTTON as i32 => mac_mouse_button_down(2),
        value if value == VK_SHIFT as i32 => {
            mac_key_down(KeyCode::SHIFT) || mac_key_down(KeyCode::RIGHT_SHIFT)
        }
        value if value == VK_CONTROL as i32 => {
            mac_key_down(KeyCode::CONTROL) || mac_key_down(KeyCode::RIGHT_CONTROL)
        }
        value if value == VK_MENU as i32 => {
            mac_key_down(KeyCode::OPTION) || mac_key_down(KeyCode::RIGHT_OPTION)
        }
        value if value == VK_LWIN as i32 => mac_key_down(KeyCode::COMMAND),
        value if value == VK_RWIN as i32 => mac_key_down(KeyCode::RIGHT_COMMAND),
        _ => mac_keycode_from_windows_vk(vk as u16)
            .map(mac_key_down)
            .unwrap_or(false),
    }
}
...
#[cfg(target_os = "macos")]
fn mac_key_down(key: CGKeyCode) -> bool {
    unsafe { CGEventSourceKeyState(CGEventSourceStateID::HIDSystemState, key) }
}

#[cfg(target_os = "macos")]
fn mac_mouse_button_down(button: u32) -> bool {
    unsafe { CGEventSourceButtonState(CGEventSourceStateID::HIDSystemState, button) }
}

#[cfg(target_os = "macos")]
fn mac_keycode_from_windows_vk(vk: u16) -> Option<CGKeyCode> {
    Some(match vk {
        0x08 => KeyCode::DELETE,
        0x09 => KeyCode::TAB,
        0x0D => KeyCode::RETURN,
