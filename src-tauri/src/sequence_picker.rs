...
mod platform {
    ...
    mod tests {
        use super::{
            classify_keyboard_message, classify_mouse_message, KeyboardHookDecision,
            MouseHookDecision,
        };
...
pub fn start_sequence_point_pick_inner(app: tauri::AppHandle) -> Result<(), String> {
    ...
    Err(String::from(
        "Sequence point picking is currently only implemented on Windows",
    ))
}

#[cfg(not(target_os = "windows"))]
pub fn cancel_sequence_point_pick_inner(app: &tauri::AppHandle) {
    use std::sync::atomic::Ordering;
