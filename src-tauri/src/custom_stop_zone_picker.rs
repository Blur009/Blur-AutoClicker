...
pub fn start_custom_stop_zone_pick_inner(app: tauri::AppHandle) -> Result<(), String> {
    ...
    Err(String::from(
        "Custom stop zone picking is currently only implemented on Windows",
    ))
}

#[cfg(not(target_os = "windows"))]
pub fn cancel_custom_stop_zone_pick_inner(app: &tauri::AppHandle) {
    use std::sync::atomic::Ordering;
