...
mod tests {
    ...
    #[test]
    fn keyboard_single_click_100_percent_duty_holds_across_cycles() {
        let mut config = sample_config();
        config.input_type = 1;
        config.key_code = b'A' as u16;
        config.duty = 100.0;

        assert!(should_hold_keyboard_across_cycles(&config));
    }

    #[test]
    fn keyboard_hold_across_cycles_requires_single_click_100_percent_duty() {
        let mut config = sample_config();
        config.input_type = 1;
        config.key_code = b'A' as u16;
        config.duty = 99.0;
        ...
        config.duty = 100.0;
        config.double_click_enabled = true;
        assert!(!should_hold_keyboard_across_cycles(&config));

        config.double_click_enabled = false;
        config.input_type = 0;
        assert!(!should_hold_keyboard_across_cycles(&config));
    ...
    #[test]
    fn keyboard_hold_repeats_every_cycle_after_initial_key_down() {
        let batch = CycleBatchPlan {
            cycles: 3,
            double_cycles: 0,
            single_cycles: 3,
            physical_clicks: 3,
        ...
        assert_eq!(keyboard_hold_repeat_count(batch, false), 2);
        assert_eq!(keyboard_hold_repeat_count(batch, true), 3);
    }
}
