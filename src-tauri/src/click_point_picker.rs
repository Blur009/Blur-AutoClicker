...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    WH_MOUSE_LL, WM_KEYDOWN, WM_MOUSEMOVE, WM_QUIT, WM_RBUTTONDBLCLK, WM_RBUTTONDOWN, WM_RBUTTONUP,
    WM_SYSKEYDOWN,
};
...
pub fn start_click_point_pick_inner(app: AppHandle) -> AppResult<()> {
    ...
    if let Err(error) = crate::overlay::show_click_point_pick_overlay(&app) {
        cancel_click_point_pick_generation(&app, generation);
        return Err(error);
    }
...
fn cancel_click_point_pick_generation(app: &AppHandle, generation: u64) {
    if stop_click_point_pick(Some(app.clone()), true, Some(generation)).is_some() {
        let _ = crate::overlay::hide_overlay(app.clone());
    }
}

fn cancel_click_point_pick_from_hook() {
    let app = stop_click_point_pick(None, true, None);
    if let Some(app) = app {
...
pub fn cancel_click_point_pick_inner(app: &AppHandle) {
    stop_click_point_pick(Some(app.clone()), true, None);
    let _ = crate::overlay::hide_overlay(app.clone());
}
    ...
    match classify_mouse_message(message, shift_down, ctrl_down) {
        ...
        MouseHookDecision::Swallow => {
            if message == WM_RBUTTONUP {
                ...
                if should_stop {
                    stop_click_point_pick(None, true, None);
                }
            }
            1
        }
